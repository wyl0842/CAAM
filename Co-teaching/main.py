# CUDA_VISIBLE_DEVICES=0 python ./Co-teaching/main.py --dataset cifar100 --noise_type symmetric --noise_rate 0.2
# CUDA_VISIBLE_DEVICES=0 python ./Co-teaching/main.py --dataset cifar100 --noise_type symmetric --noise_rate 0.2 --model_name lr0.01
# -*- coding:utf-8 -*-
import os
import torch 
...
parser.add_argument('--result_dir', type = str, help = 'dir to save result txt files', default = 'output/coteachingresults/')
parser.add_argument('--model_name', type = str, help = 'model name', default = '')
...
save_dir = args.result_dir +'/' +args.dataset+'/coteaching_%s/'%args.model_name

if not os.path.exists(save_dir):
    os.system('mkdir -p %s' % save_dir)

model_str=args.dataset+'_coteaching_%s_'%args.model_name+args.noise_type+'_'+str(args.noise_rate)
...
def main():
    ...
    cnn1.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    # print(model.conv1)
    # cnn1.conv1= nn.Conv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
    cnn1.fc = nn.Linear(512, num_classes)
    cnn1.cuda()
    print(cnn1.parameters)
    # optimizer1 = torch.optim.Adam(cnn1.parameters(), lr=learning_rate)
    optimizer1 = torch.optim.SGD(cnn1.parameters(), lr=learning_rate, weight_decay=1e-3, momentum=0.9)
    scheduler1 = MultiStepLR(optimizer1, milestones=[40, 80], gamma=0.1)
    
    # cnn2 = CNN(input_channel=input_channel, n_outputs=num_classes
    cnn2 = models.resnet18(pretrained=True)
    cnn2.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    # print(model.conv1)
    # cnn1.conv1= nn.Conv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
    cnn2.fc = nn.Linear(512, num_classes)
    cnn2.cuda()
    print(cnn2.parameters)
    # optimizer2 = torch.optim.Adam(cnn2.parameters(), lr=learning_rate)
    optimizer2 = torch.optim.SGD(cnn2.parameters(), lr=learning_rate, weight_decay=1e-3, momentum=0.9)
    scheduler2 = MultiStepLR(optimizer2, milestones=[40, 80], gamma=0.1)
