...
class CENCEandMAE(torch.nn.Module):
    def __init__(self, alpha, beta, num_classes):
        super(CENCEandMAE, self).__init__()
        self.num_classes = num_classes
    ...
    def forward(self, pred, labels):
        ...
        return ce + self.nce(pred, labels) + self.mae(pred, labels)

class AdaCENCEandMAE(torch.nn.Module):
    def __init__(self, alpha, beta, lambda1, lambda2, num_classes):
        super(AdaCENCEandMAE, self).__init__()
    ...
    def forward(self, pred, labels):
        ce = self.ce(pred, labels)
        return self.lambda1 * ce + self.lambda2 * (self.nce(pred, labels) + self.mae(pred, labels))

class AdaCEandMAE(torch.nn.Module):
    def __init__(self, alpha, beta, lambda1, lambda2, num_classes):
        super(AdaCEandMAE, self).__init__()
        self.lambda1 = lambda1
        self.lambda2 = lambda2
        self.num_classes = num_classes
        self.ce = torch.nn.CrossEntropyLoss()
    ...
    def forward(self, pred, labels):
        ce = self.ce(pred, labels)
        return self.lambda1 * ce + self.lambda2 * self.mae(pred, labels)

class AdaCEandRCE(torch.nn.Module):
    def __init__(self, alpha, beta, lambda1, lambda2, num_classes):
        super(AdaCEandRCE, self).__init__()
        self.lambda1 = lambda1
        self.lambda2 = lambda2
        self.num_classes = num_classes
        self.ce = torch.nn.CrossEntropyLoss()
    ...
    def forward(self, pred, labels):
        ce = self.ce(pred, labels)
        return self.lambda1 * ce + self.lambda2 * self.rce(pred, labels)

class AdaCEandNMAE(torch.nn.Module):
    def __init__(self, alpha, beta, lambda1, lambda2, num_classes):
        super(AdaCEandNMAE, self).__init__()
        self.lambda1 = lambda1
        self.lambda2 = lambda2
        self.num_classes = num_classes
        self.ce = torch.nn.CrossEntropyLoss()
        self.nmae = NormalizedMeanAbsoluteError(num_classes=num_classes, scale=beta)

    def forward(self, pred, labels):
        ce = self.ce(pred, labels)
        return self.lambda1 * ce + self.lambda2 * self.nmae(pred, labels)

class AdaCEandNRCE(torch.nn.Module):
    def __init__(self, alpha, beta, lambda1, lambda2, num_classes):
        super(AdaCEandNRCE, self).__init__()
        self.lambda1 = lambda1
        self.lambda2 = lambda2
        self.num_classes = num_classes
        self.ce = torch.nn.CrossEntropyLoss()
        self.nrce = NormalizedReverseCrossEntropy(num_classes=num_classes, scale=beta)

    def forward(self, pred, labels):
        ce = self.ce(pred, labels)
        return self.lambda1 * ce + self.lambda2 * self.nrce(pred, labels)

class AdaCEandNFL(torch.nn.Module):
    def __init__(self, alpha, beta, lambda1, lambda2, num_classes):
        super(AdaCEandNFL, self).__init__()
        self.lambda1 = lambda1
        self.lambda2 = lambda2
        self.num_classes = num_classes
        self.ce = torch.nn.CrossEntropyLoss()
        self.nfl = NormalizedFocalLoss(scale=alpha, gamma=0.5, num_classes=num_classes)

    def forward(self, pred, labels):
        ce = self.ce(pred, labels)
        return self.lambda1 * ce + self.lambda2 * self.nfl(pred, labels)

class AdaCEandNCE(torch.nn.Module):
    def __init__(self, alpha, beta, lambda1, lambda2, num_classes):
        super(AdaCEandNCE, self).__init__()
        self.lambda1 = lambda1
        self.lambda2 = lambda2
        self.num_classes = num_classes
        self.ce = torch.nn.CrossEntropyLoss()
    ...
    def forward(self, pred, labels):
        ce = self.ce(pred, labels)
        return self.lambda1 * ce + self.lambda2 * self.nce(pred, labels)
