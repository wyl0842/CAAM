...
from robust_loss import AdaCENCEandMAE, AdaCEandMAE, AdaCEandNMAE, AdaCEandRCE
...
######## 根据当前代的激活寻找转折点
def found_turning_point(lids, turning_epoch, name):
    init_epoch = 30
    epoch_win = 5
    if len(lids) > init_epoch + epoch_win: #
        if turning_epoch > -1: # if turning point is already found, stop checking
            ...
            if lids[-1] - np.mean(smooth_lids) > 2*np.std(smooth_lids):
                ...
                # turning_epoch = len(lids) - 15 + np.argmin(lids[-15:])
                # rollback model if you want, should be called before checkpoint callback
                # otherwise need to save two models
                min_model_path = './checkpoint/%s/%s_net%d.pth.tar'%(name,name,turning_epoch)
...
######## 调整学习策略
def update_learning_pace(A_values, p_lambda):
...
# def update_trainingloss(p_lambda):
#     if p_lambda > 1:
#         p_lambda = 1.0
#     return AdaCENCEandMAE(alpha=1.0, beta=1.0, lambda1=1.0-p_lambda, lambda2=p_lambda, num_classes=7)
#     # self.model.compile(loss=lid_paced_loss(self.alpha),
#     #                     optimizer=self.model.optimizer, metrics=['accuracy'])
