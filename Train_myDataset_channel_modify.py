...
# CUDA_VISIBLE_DEVICES=0 python Train_myDataset_channel_modify.py --id tire_res50_bootsoft_20220410_seed123_noise0.2
# CUDA_VISIBLE_DEVICES=0 python Train_myDataset_channel_modify.py --id tire_res50_adacenmae_20220519_seed123_noise0.2
# CUDA_VISIBLE_DEVICES=1 python Train_myDataset_channel_modify.py --id tire_res18_boothard_20220721_seed123_noise0.2 --model_name boot_hard
...
from util import uniform_noise_model_P, get_activation_values, found_turning_point, update_learning_pace, update_trainingloss
from loss import cross_entropy, boot_soft, boot_hard, forward, lid_paced_loss
from robust_loss import NCEandMAE, NFLandMAE, CENCEandMAE, AdaCENCEandMAE
# from loss import forward, backward
...
parser.add_argument('--lr', '--learning_rate', default=0.002, type=float, help='initial learning rate') # 0.002
...
parser.add_argument('--model_name', type=str, help='boot_hard, boot_soft, forward, backward, caam', default='robust_loss')
parser.add_argument('--loss_name', type=str, help='adacemae, adacerce', default='adacenmae')
...
parser.add_argument('--seed', default=1)#123
...
# Training
def warmup(net, optimizer, dataloader, CEloss):
    net.train()
...
#     P = uniform_noise_model_P(args.num_classes, args.noise_rate / 100.)
#     use_loss = backward(P)
elif args.model_name == 'robust_loss':
    use_loss = update_trainingloss(0.0, args.num_classes, args.loss_name)
# elif args.model_name == 'robust_ncemae':
#     use_loss = NCEandMAE(alpha=1.0, beta=1.0, num_classes=7)
# elif args.model_name == 'robust_cencemae':
#     use_loss = CENCEandMAE(alpha=1.0, beta=1.0, num_classes=7)
# elif args.model_name == 'robust_adacencemae':
#     use_loss = AdaCENCEandMAE(alpha=1.0, beta=1.0, lambda1=1.0, lambda2=0.0, num_classes=7)
...
for epoch in range(args.num_epochs+1):   
    ...
    warmup(net1, optimizer1, train_loader, use_loss)                      
    
    # val_loader = loader.run('val') # validation
    acc1 = val(net1, val_loader, epoch) 
    acc = test(net1, test_loader) 
    # log.write('Validation Epoch:%d      Acc1:%.2f    Test Accuracy:%.2f\n'%(epoch, acc1, acc))
