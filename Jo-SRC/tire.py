...
def load_data(cfg):
    ...
    if cfg.dataset=='tire':
        ...
        train_dataset = data_load.train_dataset(os.path.join(cfg.database, 'train'),
                                        True,
                                        transform=CLDataTransform(train_transform),
                                        # target_transform=tools.transform_target,
                                        # dataset=args.dataset,
                                        noise_type=cfg.noise_type,
                                        noise_rate=cfg.closeset_ratio,
                                        split_per=0.9,
                                        random_seed=123,
                                        num_class=cfg.n_classes)

        val_dataset = data_load.train_dataset(os.path.join(cfg.database, 'train'),
                                        False,
        ...
        test_dataset = data_load.test_dataset(os.path.join(cfg.database, 'test'),
...
def main(cfg, device):
    ...
    net = Model(arch=cfg.net, num_classes=n_classes, pretrained=True)  # 可以改成false
    ...
    # dataset = build_cifar10n_dataset(os.path.join(cfg.database, cfg.dataset), CLDataTransform(transform['cifar_train']), transform['cifar_test'],
